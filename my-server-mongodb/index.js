...
app.post("/login", cors({ credentials: true }), async (req, res) => {
  try {
    ...
    if (foundUser) {
      // Set cookies on successful login (7 days expiry)
      res.cookie('username', username, { maxAge: 7 * 24 * 60 * 60 * 1000, httpOnly: false });
      res.cookie('password', password, { maxAge: 7 * 24 * 60 * 60 * 1000, httpOnly: false });
...
// GET /check-login - Retrieve stored cookies
app.get("/check-login", cors({ credentials: true }), (req, res) => {
  try {
    const username = req.cookies.username || null;
    const password = req.cookies.password || null;
...
// GET /logout - Clear login cookies
app.get("/logout", cors({ credentials: true }), (req, res) => {
  try {
...
// POST /register - Create new user in MongoDB
...
// EX60
// COOKIES, SESSIONS (already set up above)

app.get("/create-cookie",cors(),(req,res)=>{
res.cookie("username","lethimyhoa")
res.cookie("password","123456")
account={"username":"lethimyhoa",
"password":"123456"}
...
//   const sampleUsers = [
//     { username: "mhoa911", password: "123456" },
...
//   const existing = await logginCollection.findOne({ username: "mhoa911" });
//   if (!existing) {
//     await logginCollection.insertOne({
//       username: "mhoa911",
//       password: "123456",
//       fullname: "Mhoa",
//       email: "mhoa911@gmail.com",
//       phone: "0123456789",
//       role: "admin",
//       createdAt: new Date(),
//     });
